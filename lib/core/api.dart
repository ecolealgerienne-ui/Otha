...
Future<List<Map<String, dynamic>>> providerAgenda({
  ...
  /// Chercher un booking actif pour un pet (pour scan QR vet)
  Future<Map<String, dynamic>?> findActiveBookingForPet(String petId) async {
    await ensureAuth();
    try {
      final res = await _dio.get('/bookings/active-for-pet/$petId');
      final data = res.data;
      if (data == null) return null;
      return _unwrap<Map<String, dynamic>?>(data);
    } catch (e) {
  ...
  Future<Map<String, dynamic>> proConfirmBooking(String bookingId) async {
    await ensureAuth();
    final res = await _dio.post('/bookings/$bookingId/pro-confirm');
    return _unwrap<Map<String, dynamic>>(res.data);
  }
  ...
  Future<Map<String, dynamic>> clientCancelBooking(String bookingId) async {
    await ensureAuth();
    final res = await _dio.post('/bookings/$bookingId/client-cancel');
    return _unwrap<Map<String, dynamic>>(res.data);
  }
  ...
  Future<List<dynamic>> getPendingValidations() async {
    await ensureAuth();
    final res = await _dio.get('/bookings/provider/me/pending-validations');
